...
### 4.1 전제와 그 검증 — ✅ 2026-07-29 실기 확인됨
...
| | 내용 |
|---|---|
| **R3** | `state==swapping` = 교체 중단. self 존재+크기>0이면 성공으로 보고 `pending`으로 승격(**`tries=0` 리셋 필수** — 직전 롤백 마커의 `tries=2`를 물려받으면 새로 깐 멀쩡한 빌드가 첫 부팅에 되돌아간다). self 없고 `.bak` 있으면 ⚠️ **`rolledback`을 먼저 기록하고 그다음 복원**한다 — R10 의 일반 규칙(‘self 가 없는 동안에는 마커 먼저, 파일 나중’)이 **복구 지점에도 똑같이 적용**된다. 순서를 뒤집으면 복원 직후 끊겼을 때 `self 존재 + .bak 없음 + pending`이 되어 '새 버전이 정상적으로 깔려 확정 대기 중'과 구별이 불가능해지고 `bad_ver`/`bad_sha`가 증발한다. self 도 `.bak`도 없고 `.sherbet-new`만 있으면 `pending`(+`tries=0`)을 먼저 기록하고 `.new → self` — 남은 유일한 소스이므로 블랙리스트 필드는 지우지 않고 그대로 둔다. **R3 과 R10 은 같은 규칙의 두 적용 지점이다. 한쪽만 고치면 반드시 어긋난다** |
...
| **R9-pre** | ⚠️ **`Sherbet-복구안내.txt`를 (다시) 기록한다.** S13 이 교체 성공 시 지웠으므로 롤백 시점에는 없다. R9a 직후부터 self 가 없고, 되돌리기까지 거부되면 자동 복구가 영영 안 도는 상태(우리 DLL 이 로드조차 안 된다)로 끝날 수 있다 — 그때 고객에게 남는 단서가 이 파일 하나다. 없으면 제품이 아무 설명 없이 사라진 것처럼 보인다(§4.4 S9 가 교체 전에 쓰는 것과 같은 이유) |
| **R9a** | `self → .sherbet-failed`(증거 보관 1개). 이 순간부터 self가 없다. 이 rename 이 거부되면 아무것도 안 옮긴 것이므로 R8 과 같이 파일은 그대로 두고 `rollback_failed`(+`bad_ver`/`bad_sha`)만 기록한다 |
...
| **R9b** | `.bak → self`. 실패하면 첫 번째를 즉시 되돌린다 — 되돌리기 **전에** `rollback_failed`를 먼저 기록한다(그 사이에 끊기면 `self=불량 바이너리 + 마커 rolledback`이 되어 거짓 안전모드가 켜진 채 고정점이 된다). 둘 다 실패면 `rollback_failed`만 남기고 조용히 계속 실행(**브릭 방지 원칙**). `bad_ver`/`bad_sha`는 되돌리기 성공 여부와 무관하게 남긴다 — '이 버전은 2회 연속 부팅에 실패했다' 는 사실은 이미 확정됐다 |
...
⚠️ **Phase 3 미결 결정 — '거부' 와 '부팅 실패' 를 구분할 것인가.** `restore_from_bak`(R3)은 되돌릴 때 `bad_ver`/`bad_sha`에 마커의 `version`(= 깔려던 새 버전)을 적는다. 그런데 그 복구가 도는 이유가 **부팅에 실패해서**가 아니라 **교체 rename 이 그 순간 거부돼서**일 수 있다(S11 이 거부되고 원복까지 거부된 뒤 다음 실행, 또는 S10\~S11 사이 전원차단). 그 경우 새 바이너리는 **설치된 적도 실행된 적도 없는데** R11 안전모드가 켜지고 `should_offer`가 그 (버전, sha)를 영구히 거부한다 — 일시적인 AV 락 한 번이 그 고객에게서 그 업데이트를 지운다. 현재 완화책은 R13 `[그래도 다시 시도]` 클릭 1회이고 비용이 그만큼이라 이번 단계에서는 **동작을 바꾸지 않는다.** Phase 3 에서 답할 질문: *교체가 '거부' 된 경우와 '부팅 실패' 인 경우를 마커에서 구분해, 전자는 블랙리스트 없이 되돌릴 것인가?* 구분하려면 마커에 필드가 하나 더 필요하고, 그러면 §5.1 포맷과 모르는 키 보존 규약을 함께 봐야 한다.
