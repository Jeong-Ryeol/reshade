...
void sherbet::update::controller::begin_update()
{
	...
	// ⚠️ 아래 두 return 은 **반드시 문구를 남긴다.** 예전에는 둘 다 조용히 돌아갔는데,
	//    그러면 사용자가 [업데이트] 를 눌렀을 때 화면에 아무 일도 안 일어난다.
	//    실제로 그 신고를 받았다("눌러도 계속 업데이트라고 뜬다").
	//    특히 페치 워커는 실패 시 15초·60초 재시도 대기를 하므로, 그동안 버튼이
	//    최대 75초간 먹통이 된다. 그걸 사용자가 알 방법이 없었다.
	if (_worker.joinable())
	{
		// "확인 중이에요 — 잠시 뒤 다시 눌러 주세요"
		set_status("\xED\x99\x95\xEC\x9D\xB8 \xEC\xA4\x91\xEC\x9D\xB4\xEC\x97\x90\xEC\x9A\x94 \xE2\x80\x94 \xEC\x9E\xA0\xEC\x8B\x9C \xEB\x92\xA4 \xEB\x8B\xA4\xEC\x8B\x9C \xEB\x88\x8C\xEB\x9F\xAC \xEC\xA3\xBC\xEC\x84\xB8\xEC\x9A\x94");
		return;
	}
	...
	{
		// "업데이트 정보를 다시 받아야 해요 — '업데이트 확인'을 눌러 주세요"
		set_status("\xEC\x97\x85\xEB\x8D\xB0\xEC\x9D\xB4\xED\x8A\xB8 \xEC\xA0\x95\xEB\xB3\xB4\xEB\xA5\xBC \xEB\x8B\xA4\xEC\x8B\x9C \xEB\xB0\x9B\xEC\x95\x84\xEC\x95\xBC \xED\x95\xB4\xEC\x9A\x94 \xE2\x80\x94 '\xEC\x97\x85\xEB\x8D\xB0\xEC\x9D\xB4\xED\x8A\xB8 \xED\x99\x95\xEC\x9D\xB8'\xEC\x9D\x84 \xEB\x88\x8C\xEB\xAC\x9C \xEC\xA3\xBC\xEC\x84\xB8\xEC\x9A\x94");
		return;
	}
	reshade::log::message(reshade::log::level::info,
		"[sherbet-update] \xEC\x82\xAC\xEC\x9A\xA9\xEC\x9E\x90\xEA\xB0\x80 [\xEC\x97\x85\xEB\x8D\xB0\xEC\x9D\xB4\xED\x8A\xB8] \xEB\x88\x8C\xEB\xA6\x84 -> %s", u.version.c_str());
