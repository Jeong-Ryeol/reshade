...
namespace sherbet
{
	namespace update
	{
		...
		//
		// ⚠️ 이 상태머신은 **파일 존재 여부만** 본다. 부팅 초기 경로(§5.2)에서 self 를
		// 해시해 '지금 이게 옛 바이너리냐 새 바이너리냐' 를 묻지 않기 때문이다. 안 묻는
		// 이유는 두 가지다: (1) 게임을 켤 때마다 로더 콜백 안에서 수 MB 를 동기로 읽는
		// 비용을 매번 치르게 된다, (2) **물을 필요가 없다** — 지금 매핑된 이미지는 자기
		// 버전(SHERBET_VERSION)을 공짜로, 정확히 알고 있고 그게 곧 §5.2 게이트1 이다.
		// (§5.2 의 제약은 '표준 라이브러리 금지' 가 아니다 — 그 문장은 틀렸고 고쳐졌다.
		//  금지는 로더 재진입을 부르는 것들이다: LoadLibrary·스레드·CNG·지연로드.
		//  이 헤더의 함수들은 §5.2 가 **호출을 요구**하는 대상이다.)
		// 그래서 판정은 다음 한 줄에만 기댄다:
